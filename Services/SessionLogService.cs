...
public static class SessionLogService
{
    ...
    public static void Initialize(string appVersion)
    {
        try
        {
            ...
            var old = Directory.GetFiles(LogDir, "TransitLab_diagnostics_*.log")
            ...
            _logPath = Path.Combine(LogDir, $"TransitLab_diagnostics_{DateTime.Now:yyyyMMdd_HHmmss}.log");
        }
        catch { return; }

        Write($"=== TransitLab {appVersion} — Session started {DateTime.Now:yyyy-MM-dd HH:mm:ss} ===");
        Write($"Config dir: {ConfigService.AppDataDir}");
        Write("");
    }
