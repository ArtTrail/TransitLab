...
///   12. Assign filter-matched magnitudes via preference chain (GSPC → APASS → G→V → VSP)
...
public static class GaiaCompService
{
    ...
    // AAVSO filter code → VSP band name. Internal (not private) so AavsoCompService can
    // reuse the same mapping for its own VSP query, keeping both band-aware in lockstep.
    internal static readonly Dictionary<string, string> VspFilterMap =
    ...
    public record CompStarInfo(
        ...
        string? GaiaId,
        string? Auid,
        string? VsxName,        // AAVSO VSX designation, if this star's position matches a VSX entry
        double  SepArcsec,
        double? BpRp,
        double? Ruwe);
        ...
        CancellationToken   ct           = default)
    {
        ...
        if (useVsp)
        {
            ...
            (vspCandidates, vspMsg) = await QueryVspAsync(
        ...
        try
        {
            (apassStars, apassMsg) = await QueryApassAsync(
        ...
        //
        // VSP + Stone: VSP stars have guaranteed priority — all valid VSP stars
        ...
        Log($"[Stone]   Magnitude sources: GSPC:{_nGspc}  APASS:{_nApass}  G→V:{_nGtoV}  VSP:{_nVsp}" +
        ...
        if (imageData != null && selected.Count > 0)
        {
            for (int pass = 0; pass < 3; pass++)
            {
                ...
                foreach (var bad in toReject)
                {
                    ...
                    // Find the next backfill candidate that passes spatial dedup against
                    // survivors. In VSP + Stone mode, a rejected slot is refilled from a VSP
                    // candidate first if one is available — otherwise a rejected VSP star could
                    // end up replaced by a Stone candidate purely because of list order, quietly
                    // breaking the "VSP always ahead of Stone" guarantee this mode promises.
                    bool TooClose(GaiaCandidate c) => selected.Any(s =>
                        s.XPixel.HasValue && s.YPixel.HasValue &&
                        c.XPixel.HasValue && c.YPixel.HasValue &&
                        Math.Abs(s.XPixel.Value - c.XPixel.Value) <= 20 &&
                        Math.Abs(s.YPixel.Value - c.YPixel.Value) <= 20);

                    int FindBackfillIndex(bool vspOnly)
                    {
                        for (int bi = 0; bi < backfill.Count - 1; bi++)  // -1 excludes the one just added
                        {
                            ...
                            if (vspOnly && candidate.Source != "VSP") continue;
                            if (!TooClose(candidate)) return bi;
                        }
                        return -1;
                    }

                    int idx = useVsp ? FindBackfillIndex(vspOnly: true) : -1;
                    if (idx < 0) idx = FindBackfillIndex(vspOnly: false);

                    if (idx >= 0)
                    {
                        var candidate = backfill[idx];
                        backfill.RemoveAt(idx);
            ...
            // A star backfilled on the LAST pass never gets measured again before the
            // loop exits (measurement happens at the top of each pass, backfill at the
            // bottom) — its Snr/Fwhm could still reflect a stale value from when it was
            // sitting in the backfill pool. Re-measure whatever ended up in `selected`
            // once more so every reported value, and the validation check below, reflects
            // what's actually being used.
            foreach (var s in selected)
            {
                if (!s.XPixel.HasValue || !s.YPixel.HasValue) continue;
                var psf = PsfService.Measure(imageData, s.XPixel.Value, s.YPixel.Value, saturationAdu, gainEPerAdu);
                s.FwhmX = psf.FwhmX; s.FwhmY = psf.FwhmY; s.FwhmPx = psf.FwhmMean;
                s.PeakAdu = psf.PeakAdu; s.Snr = psf.Snr; s.Saturated = psf.Saturated;
            }
        }

        // A "final comp star" here just means the backfill pool ran dry while trying to
        // replace rejects — it does NOT mean any of them actually cleared the SNR>=75 bar
        // above. Every candidate can fail that bar (e.g. a genuinely faint target/field)
        // and the loop still has to end with *something* selected. Track that distinction
        // explicitly so the caller can warn instead of silently reporting success.
        int validatedCount = selected.Count(c => c.Snr is > 0 and >= 75);
        bool noneValidated = imageData != null && selected.Count > 0 && validatedCount == 0;
        ...
        else if (noneValidated)                            _grade = "poor";   // forced fallback — nothing actually cleared the SNR bar
        ...
        foreach (var c in selected.Where(c => c.XPixel.HasValue && c.YPixel.HasValue))
        {
            ...
            stars.Add(new CompStarInfo(
                ...
                GaiaId:        c.GaiaId,
                Auid:          c.Auid,
                VsxName:       FindVsxMatch(c.Ra, c.Dec, vsxVariables),
        ...
        bool vspLostAll = useVsp && vspCandidates.Count > 0 && nVsp == 0;

        string src;
        if (!gaiaOk && useVsp)          src = "AAVSO VSP (Gaia unavailable)";
        else if (!gaiaOk)               src = "✗  Gaia unavailable";
        else if (!useVsp)               src = "Stone (Gaia + APASS)";
        else if (nVsp > 0 && nGaia > 0) src = $"VSP + Stone ({nVsp} VSP, {nGaia} Stone)";
        else if (nVsp > 0)              src = "VSP-only (Gaia-enriched)";
        else if (vspLostAll)            src = $"Stone (Gaia + APASS)  ⚠ {vspCandidates.Count} VSP found, 0 survived PSF check";
        ...
        if (vspLostAll)
            Log($"[Stone]    ⚠  {vspCandidates.Count} VSP candidate(s) were queried but none survived the PSF/SNR quality filter — final selection is Gaia-only.", logProgress);
        ...
        return new CompResult(pairs, msg, GaiaUsed: gaiaOk, Stars: stars, Quality: quality);
    }
    ...
    {
        // FormattableString.Invariant — without it, the numeric holes below format using the
        // OS's current culture, so a comma-decimal locale (e.g. Bulgarian, Russian, German)
        // embeds "283,306316" instead of "283.306316" into the ADQL, which the TAP server
        // rejects outright with HTTP 400 (malformed query) on every single request.
        string adql = FormattableString.Invariant($"""
    ...
    private static async Task<(List<ApassStar> Stars, string Message)> QueryApassAsync(
        double ra, double dec, double fovArcmin, string filterCode,
        CancellationToken ct)
    {
        if (!ApassFilterMap.TryGetValue(filterCode, out var colPair))
            return ([], $"Filter '{filterCode}' not mapped in APASS — skipped");
        ...
        // FormattableString.Invariant — see QueryGaiaAsync for why this matters (comma-decimal
        // locales otherwise corrupt the embedded ra/dec/radius into invalid ADQL).
        string adql = FormattableString.Invariant($"""
        ...
        if (csv is null)
            return ([], "APASS query failed on all VizieR mirrors");
    ...
    private static (List<ApassStar> Stars, string Message) ParseApassCsv(
        string csv, string magCol, string errCol)
    {
        var stars = new List<ApassStar>();
        var lines = csv.Split('\n', StringSplitOptions.RemoveEmptyEntries);
        if (lines.Length < 2) return (stars, "Empty APASS response");
        ...
        if (iRa < 0 || iDec < 0 || iMag < 0)
            return (stars, $"APASS CSV missing required columns (ra={iRa} dec={iDec} mag={iMag})");
        ...
        return (stars, $"{stars.Count} stars ({magCol})");
    }

    // ── VSP query ─────────────────────────────────────────────────────────────

    private static async Task<(List<GaiaCandidate> Stars, string Message)> QueryVspAsync(
        ...
        string filterCode, double magLimit, CancellationToken ct)
    {
        if (!VspFilterMap.TryGetValue(filterCode, out var vspFilter))
            return ([], $"Filter '{filterCode}' not in VSP — using Gaia only");

        double fov = Math.Clamp(fovArcmin * 1.2, 10.0, 90.0);
        // FormattableString.Invariant — see QueryGaiaAsync for why this matters. Must be a
        ...
        catch (Exception ex)
        {
            ...
            return ([], $"VSP request failed: {ex.Message}");
        }
        ...
        try
        {
            ...
            if (!doc.RootElement.TryGetProperty("photometry", out var phot))
            {
                ...
                return ([], "No comparison stars in VSP response");
            }
            ...
            foreach (var star in phot.EnumerateArray())
            {
                ...
                if (star.TryGetProperty("bands", out var bands))
                {
                    foreach (var b in bands.EnumerateArray())
                    {
                        ...
                        if (NumericParseService.TryParse(ms, out var mv))
                            mag = mv;
                        break;
                    }
                }
        ...
        catch (Exception ex)
        {
            ...
            return ([], $"VSP parse error: {ex.Message}");
        }

        return vspStars.Count > 0
            ? (vspStars, $"{vspStars.Count} stars (VSP {vspFilter} band)")
            : ([], "No VSP stars in this field with the selected filter");
    }
    ...
    /// Priority: GSPC synthetic photometry → APASS DR9 → G→V polynomial → VSP catalog.
    /// </summary>
        ...
        List<ApassStar>                            apassStars)
    {
        ...
        {
            // Priority 1: GSPC synthetic photometry (~0.005-0.015 mag accuracy)
            ...
            // Priority 2: APASS DR9 position match (~0.02-0.05 mag accuracy)
            ...
            // Priority 3: Hardcoded G→V polynomial (Evans et al. 2018)
            // Applied regardless of filter as a broad fallback; labelled "G→V" so user knows
            ...
            // Priority 4: VSP catalog magnitude
    ...
    private static bool TryParseAngle(string s, bool isDec, out double value)
    {
        ...
        if (NumericParseService.TryParse(s, out value))
            return true;
    ...
    private static double? ParseD(List<string> cols, int idx)
    {
        ...
        return NumericParseService.TryParse(s, out var d) ? d : null;
    }
    ...
    /// Returns the name of the nearest VSX entry within <paramref name="matchArcsec"/> of
    /// the given position, or null if none is that close. Used to label a selected comp
    /// star with its VSX designation when one happens to exist at that position (VSP stars
    /// are not run through the VSX exclusion check above, so this can find a match even for
    /// a star that made it into the final selection).
    private static string? FindVsxMatch(
        double ra, double dec, List<(double Ra, double Dec, string Name)> vsxVariables,
        double matchArcsec = 5.0)
    {
        string? best = null;
        double bestSep = matchArcsec;
        foreach (var (vRa, vDec, vName) in vsxVariables)
        {
            var sep = SepArcsec(ra, dec, vRa, vDec);
            if (sep <= bestSep) { bestSep = sep; best = vName; }
        }
        return best;
    }
